...
class VectorStore:
    ...
    def search_by_vector(
        ...
        This is identical to search() but skips the embedding step.
        Useful when:
          - You already have a vector from another source
          - You want to find records similar to an existing record
          - You're benchmarking without embedding overhead

        Args:
            query_vector: A 1D NumPy array of shape (384,) float32.
        ...
        Raises:
            ...
            ValueError: If the vector has wrong dimensions.
        """
        self._validate_top_k(top_k)
        self._require_non_empty_string(metric, "metric")
        metric_name = metric.strip().lower()
        ...
        if query_vector.ndim != 1 or query_vector.shape[0] != self.dimension:
            raise ValueError(
                f"query_vector must have shape ({self.dimension},), "
        ...
        # Determine candidates (same logic as search, minus embedding)
        if filters is not None:
            indices = self._get_filtered_indices(filters, collection=None)
            if len(indices) == 0:
                return []
            candidate_vectors = self._vectors[indices]
        else:
